...
- **Remediation of Audit Findings (H1, H2, H3, H5):**
  ...
  - **H2 (Immutable file verification):** During `--import-verdicts`, `gate_generation_eval` now verifies SHA-256 hashes of immutable artifacts (`run_manifest.json`, `outcomes.jsonl`, `retrieval.jsonl`, `blind_checklist.baseline.json`, `arm_map.sealed.json`) against `checksums.txt`, fail-closing if tampering is detected; verdict state goes to separate artifacts (`import_manifest.json`, `comparison.import.md`, `checksums.import.txt`), sealed files are never rewritten, and the import is strictly single-use.
