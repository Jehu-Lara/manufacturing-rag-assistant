...
**Note, 2026-08-25 — actual generation model in use is `openai/gpt-oss-120b` via Groq, not a Llama 3.x model as originally specified.** `GROQ_MODEL` was changed from `llama-3.3-70b-versatile` (retired by Groq, confirmed via `GET https://api.groq.com/openai/v1/models`) to `openai/gpt-oss-120b` in commit `b73e69d`. This is a model-family deviation from the Phase 3 prompt's "Groq API (Llama 3.x family)" specification, not a same-family swap — Groq's active catalog currently has no viable Llama 3.x chat model at all (the only remaining Llama-family entries are small prompt-injection-classifier models, not usable for generation). Awaiting explicit sign-off on this deviation.
