...
- The public recruiter showcase is reachable as a free Hugging Face Static Space. The interactive system targets an Oracle Cloud Always Free Ampere A1 VM and remains pending because the selected region has no available ARM capacity.
...
- **LLM generation:** Groq API (`openai/gpt-oss-120b`) is primary and OpenAI `gpt-4o-mini` is the fallback. The generation model is not hosted locally; Oracle runs retrieval, API, and UI while generation remains on the configured external provider.
...
**Status: Application implemented and locally verified; public static showcase live; interactive deployment pending.** The post-refactor implementation lives under `src/`: FastAPI in `src/main.py` plus the primary HTTP adapter/query feature, Streamlit in `src/web/`, and provider/retrieval implementations in secondary adapters. A real amd64 Docker build/run verified `/health`, `/ready`, nginx routing, and the baked index. The interactive Oracle deployment has not happened because regional ARM capacity is unavailable.
