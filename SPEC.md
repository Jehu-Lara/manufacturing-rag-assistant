...
└── docs/
    └── hf-space/                  # static HF Space content (recruiter-visibility showcase, Phase 3)
```

(This tree predates Phase 2/3 and is not exhaustive — `ui/`, `eval/`, `retrieval/`, `Dockerfile`, `nginx.conf`, etc. exist but aren't listed here; see each phase's Status section for what was actually built.)
...
**HF Static Space showcase, 2026-08-26 — built in `docs/hf-space/`, not yet pushed live.** A free Hugging Face Static Space (`sdk: static`, no PRO required — unaffected by the paywall documented above) with an architecture diagram, honest eval-results table, stack summary, and a link to the GitHub repo. The live-demo section is explicitly marked "deployment in progress" rather than left implying a working link. Real screenshots of the running UI are not yet included (no browser/screenshot tool available in this session to capture the local container; deferred rather than faked). Not yet pushed to an actual Space on HF Hub — that requires the project owner's own git push (no HF write API is available to push this programmatically).
