...
The deploy target (a Hugging Face Docker Space) runs one container with one
uvicorn worker — there is no shared store today to keep a rate limit in
sync across replicas even if there were more than one.
