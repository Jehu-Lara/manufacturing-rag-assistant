...
# pure-semantic top-1 item is present in the returned list — see
# eval.metrics.top1_semantic_score.
SEMANTIC_EXTRACTION_K = DEFAULT_TOP_N * 2


def _top1_semantic_score(question_text: str) -> float:
    results = retrieve(question_text, k=SEMANTIC_EXTRACTION_K)
...
def select_threshold(
    ...
    """Implements the plan's exact threshold-selection rule (Step 2):
...
def build_report(
    unanswerable_scores: list[float], answerable_scores: list[float], selection: dict, version: str
) -> str:
    lines = [
        ...
        (
            ...
            "(`retrieval.hybrid.RetrievalResult.semantic_score` at `semantic_rank == 1`), "
            "not the RRF `fused_score` — see SPEC.md's Phase 2 status for why `fused_score` "
...
def run() -> Path:
    ...
    answerable_scores = [_top1_semantic_score(q["question"]) for q in answerable]
    unanswerable_scores = [_top1_semantic_score(q["question"]) for q in unanswerable]
