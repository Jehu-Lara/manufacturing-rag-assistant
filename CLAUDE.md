...
- **LLM/generation module boundary**: `api/` (FastAPI backend + LLM orchestration) and `ui/` (Streamlit) are separate processes communicating over HTTP only. `ui/streamlit_app.py` must never import from `api/generation.py` (or any other `api/` module) directly, even though both live in the same container/repo — this matches the deployed architecture (nginx proxies to two independent processes) and keeps the API genuinely independently testable and reachable.
