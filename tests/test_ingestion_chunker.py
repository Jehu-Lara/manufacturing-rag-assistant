...
def test_short_sections_produce_a_single_chunk():
    chunks = chunk_document(_build_body(num_long_section_lines=5), _COUNTER)
...
def test_long_section_is_split_into_target_band_with_overlap():
    ...
    chunks = chunk_document(_build_body(num_long_section_lines=80), _COUNTER)
...
def test_chunks_never_cross_section_boundaries():
    ...
    merged_sections = _merge_small_sibling_sections(split_into_sections(body), _COUNTER)
    chunks = chunk_document(body, _COUNTER)
...
def test_count_tokens_is_positive_for_nonempty_text():
    assert _COUNTER.count_tokens("hello world") > 0
...
def test_small_sibling_sections_are_merged_into_one_chunk():
    ...
    for leaf in ("Leaf A", "Leaf B", "Leaf C"):
        assert _COUNTER.count_tokens(f"Tiny content {leaf[-1]}.") < MIN_MERGE_TOKENS
