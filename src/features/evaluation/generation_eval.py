...
# rather than threading that information through the use case we delay
...
def false_refusal_rate(rows: list[dict[str, Any]]) -> float:
...
def _build_row(use_case: QueryUseCase, retriever: HybridRetriever, question: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if question["answerable"]:
            retrieved = retriever.retrieve(question["question"], k=5)
...
def build_report(rows: list[dict[str, Any]], version: str) -> str:
    ...
    lines = [
        ...
        "",
        "Target thresholds, for reference only — this report does not compute a pass/fail verdict:",
    ...
    lines += [
        ...
        "- No per-provider breakdown: QueryAnswer does not expose which LLM provider served a request.",
        "",
...
def run(
    eval_set_path: Path = eval_set_integrity.EVAL_SET_FILE,
